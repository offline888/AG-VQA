...
class BaseModel:
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1):
        ...
        # During warmup, lr is set linearly from 0 to init_lr,
        # we must NOT advance the cosine scheduler during this period.
        if current_iter > warmup_iter:
            for scheduler in self.schedulers:
                scheduler.step()
            # Warmup is done; the cosine curve starts from its state at warmup_iter.
            # This means warmup_iter effectively counts toward T_max.
        elif warmup_iter > 0 and current_iter <= warmup_iter and current_iter > 1:
            # Warmup phase: set lr linearly from 0 to init_lr.
            init_lr_groups_l = self._get_init_lr()
            warm_up_lr_l = []
            for init_lr_g in init_lr_groups_l:
